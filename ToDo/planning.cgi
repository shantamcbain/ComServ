...
my $log_tb     = 'planning_tb';
my $TodoTB     = 'todo_tb';
my $HasMembers = 0;
my $DeBug = $CGI->param('debug')|| 0;
my $batchnumber = $CGI->param('subject')||1;
my $recipecode = $CGI->param('recipecode')||'none';
my $recipename = $CGI->param('recipename')||'none';
my $OffLine    = 'yes';
my $Affiliate = 001;
...
}if ( $CGI->param('recipecode') )
{
 if (
   $CGI->param('recipecode') ne $SESSION->getAttribute( -KEY => 'recipecode' )
   )
 {
  $SESSION->setAttribute( -KEY   => 'recipecode',
                          -VALUE => $CGI->param('recipecode') );
  $recipecode = $CGI->param('recipecode');
 }
 else
 {
  $SESSION->setAttribute( -KEY => 'recipecode', -VALUE => $recipecode  );
 }

}
else
{
 if ( $SESSION->getAttribute( -KEY => 'recipecode' ) )
 {
  $recipecode = $SESSION->getAttribute( -KEY => 'recipecode' );
 }
 else
 {
  $SESSION->setAttribute( -KEY => 'recipecode', -VALUE => $recipecode );
...
if ( $CGI->param('batchnumber') )
{
 if (
   $CGI->param('batchnumber') ne $SESSION->getAttribute( -KEY => 'batchnumber' )
   )
 {
  $SESSION->setAttribute( -KEY   => 'recipecode',
                          -VALUE => $CGI->param('batchnumber') );
  $batchnumber = $CGI->param('batchnumber');
 }
 else
 {
  $SESSION->setAttribute( -KEY => 'batchnumber', -VALUE => $batchnumber  );
 }

}
else
{
 if ( $SESSION->getAttribute( -KEY => 'batchnumber' ) )
 {
  $batchnumber = $SESSION->getAttribute( -KEY => 'batchnumber' );
 }
 else
 {
  $SESSION->setAttribute( -KEY => 'batchnumber', -VALUE => $batchnumber );
...
my $homeviewname     = 'ProjectView'||$SetupVariables->{-HOME_VIEW_NAME};
...
my $GLOBAL_DATAFILES_DIRECTORY =
  $SetupVariables->{-GLOBAL_DATAFILES_DIRECTORY} || 'BLANK';
my $TEMPLATES_CACHE_DIRECTORY = $GLOBAL_DATAFILES_DIRECTORY
  . $SetupVariables->{ -TEMPLATES_CACHE_DIRECTORY, };
...
my @ADD_FORM_DHM_CONFIG_PARAMS = (
 ...
 -FIELD_MAPPINGS => {
  ...
  site_name    => 'Owner',
  batchnumber =>
'batchnumber If not in list select other and place your suggestion in comments',
  subject =>
'Subject batchnumber  If not in list select other and place your suggestion in comments',
  share         => 'Share level',
...
my @MODIFY_FORM_DHM_CONFIG_PARAMS = (
 ...
 -FIELD_MAPPINGS => {
  site_name => 'Owner',
  batchnumber =>
'batchnumber<br> If not in list select other and place your suggestion in comments',
  subject =>
'Subject batchnumber <br> If not in list select other and place your suggestion in comments',
  share       => 'Share level',
  name        => 'Name of resource',
  description => 'Description of resource',
  url         => 'URL',
...
my @BASIC_INPUT_WIDGET_DISPLAY_ORDER = (
    qw(sitename), qw(recipecode), [qw(start_day start_mon start_year)],
    qw(batchnumber), qw(details), qw(priority), [qw(status)], qw(comments),
);
...
my @SEARCH_INPUT_WIDGET_DISPLAY_ORDER = (
    ...
    [qw(status)], qw(accumulative_time),
    qw(comments),
);
...
my @RECIPE_DATASOURCE_CONFIG_PARAMS = (
                                       -TYPE        => 'DBI',
                                       -DBI_DSN     => $DBI_DSN,
                                       -TABLE       => 'brew_recipe_tb',
                                       -USERNAME    => $AUTH_MSQL_USER_NAME,
                                       -PASSWORD    => $MySQLPW,
                                       -FIELD_NAMES => \@DATASOURCE_FIELD_NAMES,
...
my @TODO_DATASOURCE_FIELD_NAMES = qw(
  record_id
  project_code
  estimated_man_hours
  accumulative_time
...
######################################################################
#       Brew::PopulateInputWidgetDefinitionListWithRecipeWidgetAction
...
my @ACTION_HANDLER_ACTION_PARAMS = (
 ...
 -SORT_FIELD1          => $CGI->param('sort_field1')      || 'time',
