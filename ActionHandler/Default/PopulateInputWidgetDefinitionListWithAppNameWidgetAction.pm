package Default::PopulateInputWidgetDefinitionListWithAppNameWidgetAction;
# 	$Id: PopulateInputWidgetDefinitionListWithAppNameWidgetAction.pm,v 1.1 2004/01/25 03:49:21 shanta Exp shanta $	
...
sub execute {
 ...
 #                      OR group_of_poster =='CSC_admin')
 
   $cgi->param(
        -NAME  => 'raw_search', 
        -VALUE => "status=='active' 
                  AND (sitename=='$params->{-SITE_NAME}' 
                       OR sitename=='All')
                  AND list_name=='app_name'
 "
    );   
...
#        -LABELS       => \@labels
    ...
    $$input_widget_definitions{'app_name'} = \@project_code_widget;
