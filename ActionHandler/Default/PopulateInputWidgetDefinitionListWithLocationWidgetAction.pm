package Default::PopulateInputWidgetDefinitionListWithLocationWidgetAction;
# 	$Id: PopulateInputWidgetDefinitionListWithLocationWidgetAction.pm,v 1.1 2004/01/25 03:49:09 shanta Exp shanta $	
...
#AND app_name == '$params->{-MOD_NAME}'
    $cgi->param(
        -NAME  => 'raw_search', 
        -VALUE => " is_container=='1' 
...
#define tb to pick up info from
 ...
 #   my $datasource_config_params = shift @DATASOURCE_CONFIG_PARAMS;
    ...
    foreach my $record (@$ra_records) {
        my $username = $record->{'item_code'};
        push (@project_code, $username);
    }

    my @labels;
    foreach my $record (@$ra_records) {
        my $displayvalue = $record->{'item_name'};
        my $subject = $record->{'item_code'};
...
#        -LABELS       => \@labels
    ...
    $$input_widget_definitions{'location'} = \@project_code_widget;
