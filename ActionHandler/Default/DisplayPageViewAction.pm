package Default::DisplayPageViewAction;
...
sub execute {
    ...
    my $session = $params->{-SESSION_OBJECT};
        ...
        my $PageName = $cgi->param('PageName')||'TempPlate';
        my $category = $cgi->param('category');

        $cgi->param(
            -NAME  => 'raw_search',
            -VALUE => "page_code==$PageName"
        );
    
    
    if (defined($cgi->param('display_page_view'))) {
    	   ...
    	   if ($datasource_config_params) {
            ...
            my @input_widget_config_params = _rearrange
                ([
                  ...
                  -PAGE_INPUT_WIDGET_DISPLAY_ORDER
                 ],
                 [
                  -BASIC_INPUT_WIDGET_DEFINITIONS,
                  -PAGE_INPUT_WIDGET_DISPLAY_ORDER
        ...
        $app->setNextViewToDisplay(
            -VIEW_NAME => $params->{-PAGE_VIEW_NAME}
        );
