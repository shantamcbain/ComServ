package Apis::ProcessShowBillsRecordsAction;
...
sub execute {
    ...
    if (defined($cgi->param('show_bills'))) {
...
my $s_date = $cgi->param('s_date')|| '2004-01-0';
my $e_date = $cgi->param('e_date')|| '2004-12-32';
my $project_code;
if ($cgi->param('project_code')) {
$project_code = $cgi->param('project_code');
}else{
$project_code = " 'ECF_BeeWork_Bill' OR
project_code = 'ECF_Bill_Maintinance_Equipment' OR
project_code = 'ECF_Pollination_Bill' OR
project_code = 'ECF_Maintenace_Vehicles_Bills' OR
project_code = 'ECF_Bills_Maintanace_Vehicles' OR
project_code = 'ECF_Bill_Property_Matinance' OR
project_code = 'ECF_Bill_HH_WO'";
      $cgi->param(
            -NAME  => 'raw_search',
            -VALUE => "(start_date > $s_date AND start_date < $e_date)
  ...
  "
        ...
        my ($ra_records,$total_records) = $app->loadData
            (
             ...
             -SORT_FIELD1                 => 'start_date' || $params->{-SORT_FIELD1},
             -SORT_FIELD2                 => 'project_code' || $params->{-SORT_FIELD2},
