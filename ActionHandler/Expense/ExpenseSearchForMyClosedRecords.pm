package Expense::ExpenseSearchForMyClosedRecords;
...
sub execute {
    ...
    if (defined($cgi->param('view_my_closed_records'))) {
        ...
        $cgi->param(
            -NAME  => 'raw_search',
            -VALUE => "username_of_poster='$username' AND approved == '1' OR approved == 'on' OR approved == 'yes'"
        );
