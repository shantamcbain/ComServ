...
sub execute {
    ...
    if (defined($cgi->param('show_site_todo'))) {
       ...
       $app->setNextViewToDisplay(
        ...
        my $searchfield = $cgi->param('searchfield')||'project_code';
        my $display_field2 = $cgi->param('display_2');
        my $visibility = $cgi->param('visibility')||'priv';

        $cgi->param(
            -NAME  => 'raw_search',
            -VALUE =>  'project_code' == 'CAP'
        );#"category==$category""$searchfield == $searchvalue1 AND share!=$visibility"
