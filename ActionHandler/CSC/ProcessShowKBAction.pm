package CSC::ProcessShowKBAction;
...
sub execute {
    ...
    my ($params) = _rearrangeAsHash([
        ...
        -BASIC_DATA_HANDLER_MANAGER_CONFIG_PARAMS,
        -SESSION_OBJECT,
...
#    my @datamanager_config_params = _rearrange([
#           -ADD_FORM_DHM_CONFIG_PARAMS
#            ],
#            [
#           -ADD_FORM_DHM_CONFIG_PARAMS
#            ],
#        @{$params->{-BASIC_DATA_HANDLER_MANAGER_CONFIG_PARAMS}}
#    );

#    my $datahandler_manager_params = shift (@datamanager_config_params);

    if (defined($cgi->param('show_kb'))) {
       ...
       $app->setNextViewToDisplay(
        ...
        my $searchfield = $cgi->param('searchfield')||'category';
        my $display_field2 = $cgi->param('display_2');
        my $visibility = $cgi->param('visibility')||'internal';
 
        $cgi->param(
            -NAME  => 'raw_search',
            -VALUE => " company_code ==extropia AND $searchfield == $searchvalue1 AND audience !=$visibility"
        );
        ...
        my ($ra_records,$total_records) = $app->loadData
	     ...
	     -SORT_FIELD1                 => $cgi->param(-sort_field1)||$params->{-SORT_FIELD1}||'record_id' ,
	     -SORT_FIELD2                 => $cgi->param(-sort_field2) ||$params->{-SORT_FIELD2}||'questions',
             ...
             -HIDDEN_ADMIN_FIELDS_VIEW_NAME  => $params->{-HIDDEN_ADMIN_FIELDS_VIEW_NAME},
