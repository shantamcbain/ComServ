package CSC::PopulateInputWidgetDefinitionListWithSubjectWidgetAction;
...
sub execute {
    ...
    $cgi->param(
        -NAME  => 'raw_search',
        -VALUE => " status=='1' OR status=='2' OR status=='In-Process' OR status='Requested' OR status=='New'"
    );   

    my @config_params = _rearrange([
        -TODO_DATASOURCE_CONFIG_PARAMS
            ],
            [
        -TODO_DATASOURCE_CONFIG_PARAMS
            ],
...
#define tb to pick up info from
 ...
 #   my $datasource_config_params = shift @DATASOURCE_CONFIG_PARAMS;
    ...
    my $ra_records = $app->loadData((
        ...
        -SORT_DIRECTION              => 'DESC'|| $params->{-SORT_DIRECTION},
        -SORT_FIELD1                 => 'start_date',
        -SORT_FIELD2                 => 'priority',
    ...
    $$input_widget_definitions{'abstract'} = \@project_code_widget;
