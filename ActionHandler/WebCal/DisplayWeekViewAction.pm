package WebCal::DisplayWeekViewAction;
...
sub execute {
    ...
    if (!$cgi->param('DisplayMonthViewAction')&& !$cgi->param('DisplayWeekViewAction') && !$cgi->param('DisplayDayViewAction') && !$cgi->param('DisplayYearViewAction')){
           if($default_view_mode != Extropia::Core::App::WebCal::Week) {
                 return 0;
           }
    } elsif ( !$cgi->param('DisplayWeekViewAction') ) {
       return 0;   
    }
