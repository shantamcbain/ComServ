package Brew::PopulateInputWidgetDefinitionListWithItemCodeWidgetAction;
...
$GroupBoolion = $params->{-REQUIRE_MATCHING_GROUP_FOR_SEARCHING_FLAG};
 ...
 #   my $datasource_config_params = shift @DATASOURCE_CONFIG_PARAMS;
    ...
    foreach my $record (@$ra_records) {
         my $item_code = $record->{'item_code'};
         push (@item_code, $item_code);
      }
    my @labels;
    foreach my $record (@$ra_records) {
    ...
    $$input_widget_definitions{'item_code'} = \@item_code_widget;
