package BugTracker::ProcessShowAllOpenBugsPostedByUserAction;
...
sub execute {
    ...
    if (defined($cgi->param('show_all_open_bugs_posted_by_user'))) {
       ...
       $app->setNextViewToDisplay(
        ...
        $cgi->param(
            -NAME  => 'raw_search',
            -VALUE => "reporter='$username' AND status != 'Fixed'"
        );
