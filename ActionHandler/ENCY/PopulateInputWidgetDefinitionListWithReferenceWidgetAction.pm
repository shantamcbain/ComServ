package ENCY::PopulateInputWidgetDefinitionListWithReferenceWidgetAction;


# 	$Id: PopulateInputWidgetDefinitionListWithReferenceWidgetAction.pm,v 1.1.1.1 2005/06/18 20:39:32 root Exp $	
...
#define tb to pick up info from
 ...
 #   my $datasource_config_params = shift @DATASOURCE_CONFIG_PARAMS;
    ...
    foreach my $record (@$ra_records) {
        my $username = $record->{'url'}." ".$record->{'title'}." ".$record->{'pages'};
...
#        my $values = "a HREF='".$record->{'url'}."'&gt;".$record->{'title'}."< ".$record->{'pages'};
         my $values = $record->{'reference_code'};
       push (@labels,$values => $displayvalue);
    }
    my %labels = @labels;
...
#        -LABELS       => \%labels,
    ...
    $$input_widget_definitions{'reference'} = \@project_code_widget;
