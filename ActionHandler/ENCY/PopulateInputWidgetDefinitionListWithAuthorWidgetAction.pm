package ENCY::PopulateInputWidgetDefinitionListWithAuthorWidgetAction;


# 	$Id: PopulateInputWidgetDefinitionListWithAuthorWidgetAction.pm,v 1.1.1.1 2005/06/18 20:39:32 root Exp $	
...
#define tb to pick up info from
 ...
 #   my $datasource_config_params = shift @DATASOURCE_CONFIG_PARAMS;
    ...
    my @labels;
    foreach my $record (@$ra_records) {
        my $displayvalue = $record->{'fname'}.' '.$record->{'lname'};
...
#        -LABELS       => \@labels
    ...
    $$input_widget_definitions{'author_code'} = \@project_code_widget;
