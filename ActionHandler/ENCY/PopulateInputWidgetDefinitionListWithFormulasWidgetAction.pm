package ENCY::PopulateInputWidgetDefinitionListWithFormulasWidgetAction;
...
sub execute {
    ...
    my @config_params = _rearrange([
        -FORMULAS_DATASOURCE_CONFIG_PARAMS
            ],
            [
        -FORMULAS_DATASOURCE_CONFIG_PARAMS
            ],
...
#define tb to pick up info from
 ...
 #   my $datasource_config_params = shift @DATASOURCE_CONFIG_PARAMS;
    ...
    foreach my $record (@$ra_records) {
        my $username = "formulas.cgi?".$record->{'formula_code'};
    ...
    $$input_widget_definitions{'formulas'} = \@project_code_widget;
