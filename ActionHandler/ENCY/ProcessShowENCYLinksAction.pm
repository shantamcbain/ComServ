package ENCY::ProcessShowENCYLinksAction;
...
sub execute {
    ...
    my @datamanager_config_params = _rearrange([
        ...
        @{$params->{-URL_DATA_HANDLER_MANAGER_CONFIG_PARAMS}}
    );

    my $datahandler_manager_params = shift (@datamanager_config_params);

    if (defined($cgi->param('show_devel_urls'))) {
       ...
       $app->setNextViewToDisplay(
        ...
        my $searchvalue1 = $cgi->param('search1'||'category');
        ...
        $cgi->param(
            -NAME  => 'raw_search',
            -VALUE => "$searchfield == $searchvalue1 and share!=$visibility"
        );
        ...
        my ($ra_records,$total_records) = $app->loadData
            (
             ...
             -DATA_HANDLER_MANAGER_CONFIG_PARAMS => $datahandler_manager_params,
