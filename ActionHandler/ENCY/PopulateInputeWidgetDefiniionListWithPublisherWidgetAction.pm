package ENCY::PopulateInputWidgetDefinitionListWithPublisherWidgetAction;


# 	$Id: PopulateInputeWidgetDefiniionListWithPublisherWidgetAction.pm,v 1.1.1.1 2005/06/18 20:39:32 root Exp $	
...
sub execute {
    ...
    my @config_params = _rearrange([
        -ADDRESS_DATASOURCE_CONFIG_PARAMS
            ],
            [
        -ADDRESS_DATASOURCE_CONFIG_PARAMS
            ],
...
#define tb to pick up info from
 ...
 #   my $datasource_config_params = shift @DATASOURCE_CONFIG_PARAMS;
    ...
    foreach my $record (@$ra_records) {
        my $username = $record->{'record_id'};
        push (@project_code, $username);
    }

    my @labels;
    foreach my $record (@$ra_records) {
        my $displayvalue =$record->{'company_name'};
        my $subject = $record->{'record_id'};
...
#        -LABELS       => \@labels
    ...
    $$input_widget_definitions{'publisher_code'} = \@project_code_widget;
