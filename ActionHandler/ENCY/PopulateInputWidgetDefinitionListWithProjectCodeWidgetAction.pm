package CSC::PopulateInputWidgetDefinitionListWithProjectCodeWidgetAction;
...
sub execute {
    ...
    $cgi->param(
        -NAME  => 'raw_search',
        -VALUE => "status=='Requested' OR status=='In-Process' OR status=='Testing' OR status=='Delivered'"
    );  
