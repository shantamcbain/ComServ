...
#----------------------------------------------------------------------------
#
# $Id: Document.pm,v 1.7 2001/09/09 15:23:51 gunther Exp $
#
#============================================================================
...
# template to a file, specified by name as the first parameter.  The
# second parameter should be a hash array containing a main template
# BLOCK, a hash array of additional DEFBLOCKS (named BLOCKs definined
# in the template document source) and a hash array of METADATA items.
# The values for the BLOCK and individual BLOCKS entries should be
# strings containing Perl code representing the templates as compiled
# by the parser.
#
# Returns 1 on success.  On error, sets the $ERROR package variable to
# contain an error message and returns undef.
#
# This is a bit of an ugly hack.  It might be better if the Document
# object itself had an as_perl() method to return a Perl
# representation of itself.  But that would imply it had to store it's
# Perl text as well as a reference to the evaluated Perl sub-routines.
# Using this approach, we can let the new() constructor eval() the
# Perl code and then discard the source text.
#------------------------------------------------------------------------
