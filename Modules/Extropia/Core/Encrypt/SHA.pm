#$Id: SHA.pm,v 1.1.1.1 2001/03/12 05:38:31 stas Exp $
...
package Extropia::Core::Encrypt::SHA;
...
sub encrypt {
    ...
    eval { require SHA; };
    if ($@) {
        die ("SHA Module as not found for loading. $@");
    }
    if ($self->{-KEY}) {
        $content_to_encrypt .= $self->{-KEY};
    }
    my $hash =  SHA->hexhash($content_to_encrypt);
    $hash =~ s/ //g;
    return $hash;
}
