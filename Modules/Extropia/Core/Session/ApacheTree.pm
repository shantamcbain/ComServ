#$Id: ApacheTree.pm,v 1.2 2001/05/21 08:45:31 gunther Exp $
...
package Extropia::Core::Session::ApacheTree;
...
sub new {
    ...
    my @dir_levels;
    if ($self->{-DIR_LEVELS}) {
        @dir_levels = (DirLevels => $self->{-DIR_LEVELS});
        Apache::Session::TreeStore->tree_init(
                    $self->{-SESSION_DIR},
                    $self->{-DIR_LEVELS})
    }
    ...
    my %tied_hash;
    tie %tied_hash, 'Apache::Session::Tree',
    ...
    if (defined %tied_hash) {
        ...
        die("Failed to create a tied hash for ApacheTree " . 
...
sub _getSessionFilename {
    ...
    my $levels = $self->{-DIR_LEVELS};

    my $file = $self->{-SESSION_ID};
    while ($levels) {
        $file =~ s|((..){$levels})|$1/|;
        $levels--;
    }

    return ($self->{-SESSION_DIR} . '/' . $file);
}
...
sub _getSessions {
    ...
    my ($session_params, @other_args) = 
        _rearrangeAsHash([-SESSION_DIR,-DIR_LEVELS], [], @_);

    my $session_dir = $session_params->{-SESSION_DIR} || 
        Extropia::Core::UniqueFile::findTempDirectory();

    my $dir_levels = $session_params->{-DIR_LEVELS} || 0;

# get possible directory list to traverse...
    my @session_prefixes;
    my $levels = $dir_levels;
    my $digitwidth = sprintf("%02d",$levels * 2);
    my $session_prefix;
    my $dir_counter; 
    for ($dir_counter = 0; $dir_counter < 256 ** $levels; $dir_counter++) {
        $session_prefix = sprintf("%${digitwidth}x",$dir_counter);      
        push(@session_prefixes, $session_prefix);
    }
    @session_prefixes = ("") if (@session_prefixes == 1);

    my $directory = "";
    my @session_file_list;
    foreach $session_prefix (@session_prefixes) {
        if ($session_prefix) {
            $directory = $session_prefix;
            $directory =~ s/(\w{2})\B/$1\//g;
            $directory = "/" . $directory;
        }
        my $full_path = $session_dir . $directory . "/";
        local *SESSIONDIR;
        opendir(SESSIONDIR, $full_path) ||
            confess("ApacheTree Session Package could not open $full_path " .
                    "to getSessions. $!");
        push(@session_file_list, map { $session_prefix . $_ } grep(!/^\.+$/, readdir(SESSIONDIR)));
        closedir (SESSIONDIR);
    }
